...
| Small Team with Complex Architecture | Team ≤ 3 AND Microservices AND multiple DBs/queues | 🟡 Medium | Complex architecture with small team adds overhead. | 1. Simplify 2. Managed services 3. Keep complex |

**Context-Based Severity Adjustments**:
- **Team Size**: Small (1–3) → complexity UP; Large (9+) → DOWN
- **Scope**: MVP → over-engineering UP; Enterprise → under-engineering UP
- **Timeline**: Urgent (<3mo) → complexity UP; Long-term (>6mo) → DOWN
