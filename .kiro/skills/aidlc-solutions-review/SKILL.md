...
1. `.kiro/` → Kiro. `SPECS_DIR=.kiro/specs`, `STEERING_DIR=.kiro/steering`, `SKILL_DIR=.kiro/skills/aidlc-solutions-review`
...
4. Resolve design documents:
   ...
   - **From conventional paths**: scan `{SPECS_DIR}/{feature}-*/design.md` for unit design folders
...
1. {Action — resolve critical issue CR-1}
2. {Action — resolve critical issue CR-2}

### Design Refinements (Should Do)
1. {Refinement — address major issue MJ-1}

### Consolidation Opportunities (Nice to Have)
1. {Opportunity — address minor issue MN-1}
...
- CR-1: {brief description} — affects {units}
- CR-2: {brief description} — affects {units}

---
🔲 **Your turn**:
- ✏️ "fix CR-1" — update the affected unit's design (activates aidlc-design for that unit)
...
- **"proceed"**: Update manifest (add `architecture-review` to workflow artifacts). Recommend next step based on manifest state (typically: activate `aidlc-tasks` for the next unit, or select next unit via `aidlc-foundation`).
...
- Tools — Kiro: `readMultipleFiles`, `fsWrite`. Claude Code: parallel `Read`, `Write`. Cursor/Windsurf: sequential reads, `Write`.
