...
Where {designDir} = `{SPECS_DIR}/{feature}` (comprehensive) or `{SPECS_DIR}/{feature}-{unit}` (incremental).
...
{If incremental: "Unit: {unit}. Unit artifacts at: {SPECS_DIR}/{feature}-{unit}/"}
...
4. Update manifest: update `implementation.completedTasks` with all tasks from this wave
